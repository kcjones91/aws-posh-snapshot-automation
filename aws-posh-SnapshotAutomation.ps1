...
if (-not $SnapshotName -or -not $Description) {
    Write-Output "Usage: .\aws-createEBSsnapshot.ps1 -SnapshotName <Name> -Description <Description> [-InstanceIP <IP Address>] [-VolumeId <VolumeId>]"
    exit 1
}
...
if (-not $InstanceIP -and -not $VolumeId) {
 ...
 if (-not $Region) {
    Write-Output "You must provide a region with -Region <region>"
 }
...
if ($VolumeId) {
    ...
    try {
        $Snapshot = New-EC2Snapshot -VolumeId $VolumeId -Region $Region -Description $Description 
...
if ($InstanceIP) {
    ...
    try {
        $Instance = Get-EC2Instance -Filter @{Name="private-ip-address"; Values="$InstanceIP"} -Region $Region |
        ...
        $Volumes = Get-Ec2Volume -Filter @{Name="attachment.instance-id"; Values="$InstanceId"} -Region $Region
        if (-not $Volumes) {
    ...
    foreach ($Volume in $Volumes) {
        ...
        Write-Output "Creating snapshot for volume ID: $VolumeId with description: '$Description'..."
        try {
            $Snapshot = New-EC2Snapshot -VolumeId $VolumeId -Region $Region -Description "$Description - Volume: $VolumeId"
            ...
            do {
                Start-Sleep -Seconds 5
                $SnapshotStatus = Get-EC2Snapshot -SnapshotId $SnapshotId -Region $Region
                $Progress = $SnapshotStatus.Progress
                $State = $SnapshotStatus.State
                Write-Output "Snapshot Progress: $Progress%, State: $State"
            } while ($State -ne "completed")
            ...
            $InstanceTags = Get-EC2Tag -Filter @{Name="resource-id"; Values=$InstanceId} -Region $Region
            if ($InstanceTags) {
            ...
            # Add Snapshot Name Tag
            New-EC2Tag -Resource $SnapshotId -Tags @{Key="Name"; Value="$SnapshotName - $VolumeId"} -Region $Region
